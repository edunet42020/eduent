...
$data=mysqli_query($con,"SELECT * FROM `tblcategory` WHERE category_id=$ccategoryid");
$rowdata=mysqli_fetch_array($data);
$ccategoryname=$rowdata['category_name'];
$st=$row['course_status'];
$startdate=$row['course_start_date'];
$enddate=$row['course_end_date'];
$sd=$row['course_start_date'];
                            ...
                            <div class="kt-subheader__main">
                                ...
                                <div class="kt-subheader__group" id="kt_subheader_search">
                                    <span class="kt-subheader__desc" id="kt_subheader_total">
                                        Change Course details and submit </span>
                                                                    ...
                                                                    <div class="form-group row">
                                                                            <label
                                                                                ...
                                                                                <div class="col-lg-9 col-xl-6">
                                                                                    <div class="kt-avatar kt-avatar--outline kt-avatar--circle- kt-avatar--changed" id="kt_user_edit_avatar">
                                                                                        <div class="kt-avatar__holder"  ><img  src="<?php echo "../../../../../../admin/themes/metronic/theme/default/demo1/dist/assets/media/course/".$photo; ?>" height="120" width="120" id="image" /></div>
                                                                        ...
                                                                        <div class="form-group row">
                                                                            <label
                                                                                ...
                                                                                <input type="text" readonly id="ccategory" name="ccategory" value="<?php echo $ccategoryname;  ?>" class="btn btn-outline-primary">
                                                                        ...
                                                                        <div class="form-group row">
                                                                            ...
                                                                            <div class="col-lg-9 col-xl-9">
                                                                                ...
                                                                                placeholder="Enter Basic Description" value="<?php echo $row['course_description']; ?>">
                                                                        ...
                                                                        <div class="form-group row">
                                                                            ...
                                                                            <div class="col-lg-9 col-xl-9">
                                                                                <textarea class="form-control" rows="3" id="cfulldescription" name="cfulldescription"  placeholder="Enter Detailed Description  "><?php echo $row['course_full_description']; ?></textarea>
...
$select="select * from tblweek where course_id=1011";
$res=mysqli_query($con,$select);
while($result=mysqli_fetch_array($res))
{
    if ($sd > $coursestartdate)
    {
        $date=date_create($result['start_date']);
        date_sub($date,date_interval_create_from_date_string("$days days"));
        $sd=date_format($date,"Y-m-d");
   
        $dateend=date_create($result['end_date']);
        date_sub($dateend,date_interval_create_from_date_string("$days days"));
        $ed=date_format($dateend,"Y-m-d");
    }
    else
    {
        $date=date_create($result['start_date']);
        ...
        $dateend=date_create($result['end_date']);
        date_add($dateend,date_interval_create_from_date_string("$days days"));
        $ed=date_format($dateend,"Y-m-d");
    }
    $wi=$result['week_id'];
    
    $up="update tblweek set start_date='$sd',end_date='$ed' where week_id=$wi";
    $res1=mysqli_query($con,$up);
                            ...
                            if(mysqli_query($con,$insert) or die(mysqli_error($con)))
                            {
                                ...
                                Swal.fire({
                                    ...
                                    window.location ='http://localhost/edunet/admin/preview/demo1/crud/metronic-datatable/advanced/allcourse.php';
                                });
                                </script>
                        ...
                        {
                            $insert = "UPDATE `tblcourse` set course_name='$cname', course_description='$cdescription', course_full_description='$cfulldescription', course_status='$coursestatus',course_start_date='$coursestartdate',course_end_date='$courseenddate',course_prize='$courseprice' where course_id=$cid";
                            $date = strtotime($sd);
$d = strtotime($coursestartdate);
$diff = abs($d - $date);
$years = floor($diff / (365*60*60*24));   
...
$select="select * from tblweek where course_id=$course_id";
//echo $select;
$res=mysqli_query($con,$select);
while($result=mysqli_fetch_array($res))
{
   // $d=7;
    //$date=strtotime($result['start_date']);
    if ($sd > $coursestartdate)
    {
        $date=date_create($result['start_date']);
        date_sub($date,date_interval_create_from_date_string("$days days"));
        $sd=date_format($date,"Y-m-d");
        //echo "             dkdsnfjdsbfdsfsddfsdtgdf","start date changed:"."$sd"; 
        $dateend=date_create($result['end_date']);
        date_sub($dateend,date_interval_create_from_date_string("$days days"));
        $ed=date_format($dateend,"Y-m-d");
       // echo "             dkdsnfjdsbfdsfsddfsdtgdf","end date changed:"."$ed"; 
    }
    else
    {
        $date=date_create($result['start_date']);
