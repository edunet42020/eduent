...
$query = "SELECT * FROM tblcreditexam ORDER BY ce_id LIMIT $start_from, $record_per_page";

if(empty($search))
{
if (isset($_POST["sort"])) {
  $dat=$_POST["sort"];
  $query = "SELECT * FROM tblcreditexam ORDER BY $dat LIMIT $start_from, $record_per_page";
...
else
{
  ...
  $query = "SELECT * FROM `tblcreditexam` WHERE CONCAT(`ce_id`,`ce_question`,`ce_op1`,`ce_op2`,`ce_op3`,`ce_op4`,`ans`,`ce_date `) LIKE '%$search%' ORDER BY $dat LIMIT $start_from,$record_per_page";
  ...
  <tbody class="kt-datatable__body" style="max-height: 230px; overflow: auto;">
    ...
    ?>
      <tr data-row="0" class="kt-datatable__row center" style="left: 0px;">
        ...
        <td class="kt-datatable__cell "><span ><?php echo $row["ce_id"]; ?></span>
        </td>
        
        <td class="kt-datatable__cell "><span><?php echo $row["ce_question"]; ?></span>
        </td>
       <!-- 
        <td  class="kt-datatable__cell"><span ><#?php echo $row["question_type"]; ?></span>
        </td>
    -->
        <td  class="kt-datatable__cell "><span ><?php echo $row["ce_op1"]; ?></span>
        </td>
        <td  class="kt-datatable__cell "><span ><?php echo $row["ce_op2"]; ?></span>
        </td>
        <td  class="kt-datatable__cell "><span ><?php echo $row["ce_op3"]; ?></span>
        </td>
        <td  class="kt-datatable__cell "><span ><?php echo $row["ce_op4"]; ?></span>
        </td>
        <td  class="kt-datatable__cell"><span ><?php echo $row["ans"]; ?></span>
        </td>
        <td  class="kt-datatable__cell"><span ><?php echo $row["ce_date"]; ?></span>
        </td>
...
-->
             <a href="http://localhost/edunet/admin/preview/demo1/custom/apps/user/edit-credit.php?id=<?php echo $cpid;?>" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit details">
              <i class="flaticon2-file"></i> </a>
...
<div align="center">
  ...
  <?php
  $page_query = "SELECT * FROM tblcreditexam ORDER BY ce_id";
  ...
  //{
   // $id=$_GET['id'];
    //echo "<script> alert('delete from tblcreditexam where q_id=$id');</script>";
    //$deletequery = "delete from tblcreditexam where q_id=$id";
...
if(isset($_POST["delid"])){
 ...
 // echo "<script>alert('urvesh');</script>";
  ...
  $deletequery = "delete from tblcreditexam where ce_id='$deleteid'";
