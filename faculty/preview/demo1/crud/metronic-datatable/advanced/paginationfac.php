...
/*if ($sortcat == "id") {
  if (isset($search)) {
    $query = "SELECT * FROM `tblfaculty` WHERE CONCAT(`f_id`, `f_name`, `f_ph_no`, `f_description`, `f_photo_path`, `f_qualification`) LIKE '%$search%' ORDER BY f_id LIMIT $start_from,$record_per_page";
  } else {
    $query = "SELECT * FROM tblfaculty ORDER BY f_id LIMIT $start_from, $record_per_page";
  }
} elseif ($sortcat == "firstname") {
  if (isset($search)) {
    $query = "SELECT * FROM `tblfaculty` WHERE CONCAT(`f_id`, `f_name`, `f_ph_no`, `f_description`, `f_photo_path`, `f_qualification`) LIKE '%$search%' ORDER BY u_first_name LIMIT $start_from,$record_per_page";
  } else {
    $query = "SELECT * FROM tblfaculty ORDER BY u_fist_name LIMIT $start_from, $record_per_page";
  }
} elseif ($sortcat == "middlename") {
  if (isset($search)) {
    $query = "SELECT * FROM `tblfaculty` WHERE CONCAT(`f_id`, `f_name`, `f_ph_no`, `f_description`, `f_photo_path`, `f_qualification`) LIKE '%$search%' ORDER BY u_middle_name LIMIT $start_from,$record_per_page";
  } else {
    $query = "SELECT * FROM tblfaculty ORDER BY u_middle_name LIMIT $start_from, $record_per_page";
  }
} elseif ($sortcat == "lastname") {
  if (isset($search)) {
    $query = "SELECT * FROM `tblfaculty` WHERE CONCAT(`f_id`, `f_name`, `f_ph_no`, `f_description`, `f_photo_path`, `f_qualification`) LIKE '%$search%' ORDER BY u_last_name LIMIT $start_from,$record_per_page";
  } else {
    $query = "SELECT * FROM tblfaculty ORDER BY u_last_name LIMIT $start_from, $record_per_page";
  }
} elseif ($sortcat == "phptopath") {
  if (isset($search)) {
    $query = "SELECT * FROM `tblfaculty` WHERE CONCAT(`f_id`, `f_name`, `f_ph_no`, `f_description`, `f_photo_path`, `f_qualification`) LIKE '%$search%' ORDER BY u_photo_path LIMIT $start_from,$record_per_page";
  } else {
    $query = "SELECT * FROM tblfaculty ORDER BY u_photo_path LIMIT $start_from, $record_per_page";
  }
} elseif ($sortcat == "phptopath") {
  if (isset($search)) {
    $query = "SELECT * FROM `tblfaculty` WHERE CONCAT(`f_id`, `f_name`, `f_ph_no`, `f_description`, `f_photo_path`, `f_qualification`) LIKE '%$search%' ORDER BY u_ph_no LIMIT $start_from,$record_per_page";
  } else {
    $query = "SELECT * FROM tblfaculty ORDER BY u_ph_no LIMIT $start_from, $record_per_page";
  }
} else {
  if (isset($search)) {
    $query = "SELECT * FROM `tblfaculty` WHERE CONCAT(`f_id`, `f_name`, `f_ph_no`, `f_description`, `f_photo_path`, `f_qualification`) LIKE '%$search%' LIMIT $start_from,$record_per_page";
  } else {
    $query = "SELECT * FROM tblfaculty LIMIT $start_from, $record_per_page";
  }
}
  ...
  <tbody class="kt-datatable__body" style="max-height: 297px; overflow: auto;">
    ...
    ?>
      <tr data-row="0" class="kt-datatable__row" style="left: 0px;">
        ...
        <!--<td class="kt-datatable__cell--center kt-datatable__cell kt-datatable__cell--check" data-field="RecordID"><span style="width: 20px;"><label class="kt-checkbox kt-checkbox--single kt-checkbox--solid"><input type="checkbox" class="delct" name="language[]" id="language1" value="<?php  echo $_POST['f_id']  ?>">&nbsp;<span></span></label></span></td>
        -->
