...
$query = "SELECT * FROM tblweek as w ,tblcourse as c where w.course_id=c.course_id and c.course_t_id=$facultyid ORDER BY week_id LIMIT $start_from, $record_per_page";
/*
if (isset($search)) {
...
if (isset($_POST["sort"])) {
  $dat=$_POST["sort"];
  $query = "SELECT * FROM tblweek as w ,tblcourse as c where w.course_id=c.course_id and c.course_t_id=$facultyid ORDER BY $dat LIMIT $start_from, $record_per_page";
...
else
{
  ...
  $query = "SELECT * FROM `tblweek` as w ,tblcourse as c  WHERE CONCAT(`week_id`, `week_title`, `chap_name`, `start_date`, `end_date`, `course_name`) LIKE '%$search%' and w.course_id=c.course_id and c.course_t_id=$facultyid ORDER BY $dat LIMIT $start_from,$record_per_page";
}
//echo $query;
$result = mysqli_query($connect, $query);
    ...
    ?>
      <tr data-row="0" class="kt-datatable__row" style="left: 0px;">
        ...
        <td class="kt-datatable__cell--left kt-datatable__cell" data-field="Actions" data-autohide-disabled="false"><span style="overflow: visible; position: relative;">
             <a href="http://localhost/edunet/faculty/preview/demo1/custom/apps/user/edit-week.php?id=<?php echo $cpid;?>" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit details">
