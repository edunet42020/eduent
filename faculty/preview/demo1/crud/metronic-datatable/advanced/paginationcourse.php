...
$query = "SELECT * FROM tblcourse WHERE course_t_id ='$facultyid' ORDER BY course_id LIMIT $start_from, $record_per_page";

if(empty($search))
{
if (isset($_POST["sort"])) {
  $dat=$_POST["sort"];
  $query = "SELECT * FROM tblcourse WHERE course_t_id ='$facultyid' ORDER BY $dat LIMIT $start_from, $record_per_page";
...
else
{
  ...
  $query = "SELECT * FROM `tblcourse` WHERE CONCAT(`course_id`, `course_name`, `course_description`,`course_full_description`, `course_status`,`course_start_date`,`course_end_date`, `course_prize`) LIKE '%$search%' AND course_t_id ='$facultyid' ORDER BY $dat LIMIT $start_from,$record_per_page";
...
<table class="kt-datatable__table" style="display: block; max-height: 390px;">
  <thead class="kt-datatable__head">
    <tr class="kt-datatable__row" style="left: 0px;">
      ...
      <th data-field="Type" data-autohide-disabled="false" class="kt-datatable__cell kt-datatable__cell--sort">
        <span ><a href="#" class="btn btn-outline-primary" name='sortnumber' onclick="load_data('1','course_status','<?php if (isset($search)) { echo "$search";} ?>');">Status&nbsp;<span class="fas fa-sort"></span></a></span>
      </th>
      <th data-field="Type" data-autohide-disabled="false" class="kt-datatable__cell kt-datatable__cell--sort">
        <span ><a href="#" class="btn btn-outline-primary" name='sortnumber' onclick="load_data('1','course_start_date','<?php if (isset($search)) { echo "$search";} ?>');">Start Date&nbsp;<span class="fas fa-sort"></span></a></span>
      </th>
      <th data-field="Type" data-autohide-disabled="false" class="kt-datatable__cell kt-datatable__cell--sort">
  ...
  <tbody class="kt-datatable__body" style="max-height: 297px;  overflow:scroll;">
    <?php
    while ($row = mysqli_fetch_array($result)) {
      $cpid = $row["course_id"];

      $imagepath="../../../../../../admin/themes/metronic/theme/default/demo1/dist/assets/media/course/".$row['course_photo_path'];
    ...
    ?>
      <tr data-row="0" class="kt-datatable__row" style="left: 0px;">
        ...
        <td class="kt-datatable__cell--left kt-datatable__cell" data-field="Actions" data-autohide-disabled="false"><span style="overflow: visible; position: relative;">
             <a href="http://localhost/edunet/faculty/preview/demo1/custom/apps/user/edit-course.php?id=<?php echo $cpid;?>" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit details">
...
<div align="center">
  ...
  <?php
  $page_query = "SELECT * FROM tblcourse WHERE course_t_id ='$facultyid' ORDER BY course_id";
...
if(isset($_POST["delid"])){
  ...
  {
    $filepath="../../../../../../admin/themes/metronic/theme/default/demo1/dist/assets/media/course/".$row['course_photo_path'];
  }
    $unimage=$filepath;
