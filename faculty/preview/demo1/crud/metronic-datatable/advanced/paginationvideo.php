...
$query = "SELECT * FROM tblvideo WHERE V_F_ID = $facultyid ORDER BY v_id LIMIT $start_from, $record_per_page";
/*
if (isset($search)) {
...
if (isset($_POST["sort"])) {
  $dat=$_POST["sort"];
  $query = "SELECT * FROM tblvideo WHERE V_F_ID = $facultyid ORDER BY $dat LIMIT $start_from, $record_per_page";
...
else
{
  ...
  $query = "SELECT * FROM `tblvideo` as v,tblfaculty as f WHERE CONCAT(`v_id`, `v_name`,`v_description`,`v_course_id`,`v_week_id`,`f_name`) LIKE '%$search%' AND V_F_ID = $facultyid ORDER BY $dat LIMIT $start_from,$record_per_page";
}
...
<table class="kt-datatable__table" style="display: block; max-height: 350px;">
  ...
  <tbody class="kt-datatable__body" style="max-height: 297px; overflow: auto;">
    ...
    while ($row = mysqli_fetch_array($result)) {
      $cpid = $row["v_id"];
      $imagepath="../../../../../../admin/themes/metronic/theme/default/demo1/dist/assets/media/video/".$row['v_path'];
      $qa="SELECT * FROM tblcourse WHERE course_id='$row[v_course_id]'";
      $qb="SELECT * FROM tblfaculty WHERE f_id='$facultyid'";
      ...
      <tr data-row="0" class="kt-datatable__row" style="left: 0px;">
       ...
       <td class="kt-datatable__cell--left kt-datatable__cell" data-field="Actions" data-autohide-disabled="false">
          <span style="overflow: visible; position: relative; width: 110px;">
            <a href="http://localhost/edunet/faculty/preview/demo1/custom/apps/user/edit-video.php?id=<?php echo $cpid;?>" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit details">
              <i class="flaticon2-file"></i> </a> </span>
...
if(isset($_POST["delid"])){
  ...
  {
    $filepath="../../../../../../admin/themes/metronic/theme/default/demo1/dist/assets/media/video/".$row['v_path'];
  }
